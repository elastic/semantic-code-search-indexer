...
3.  **Configuration**: For the indexing script, allow the user to provide configuration via environment variables. For Elasticsearch: `SCSI_ES_ENDPOINT`, `SCSI_ES_USERNAME`, `SCSI_ES_PASSWORD`, or if the user provides `SCSI_ES_API_KEY`, then use that instead of the username and passwords.
