...
export class SqliteQueue implements IQueue {
  ...
  async requeueStaleTasks(): Promise<void> {
    ...
    const distinctPids = this.db
      ...
      FROM queue
      WHERE status = '${QUEUE_STATUS_PROCESSING}'
      AND worker_pid IS NOT NULL
    `
      )
      .all() as { worker_pid: number }[];
      ...
      for (let i = 0; i < deadPids.length; i += BATCH_SIZE) {
        ...
        const result = this.db
          .prepare(
            `
          UPDATE queue
          SET status = '${QUEUE_STATUS_PENDING}',
    ...
    const result = this.db
      ...
      AND (
        ...
        OR datetime(processing_started_at) < datetime('now', '-${staleMinutes} minutes')
      )
    `
      )
      .run();
    ...
    const countResult = this.db
      .prepare(`SELECT COUNT(*) as count FROM queue WHERE status = '${QUEUE_STATUS_PROCESSING}'`)
      .get() as { count: number };
